...
use crate::mcp::config::{Settings, McpServerConfig, ProviderConfig, ProviderType, ModelConfig};
...
async fn list_conversations(state: State<'_, AppState>) -> Result<Vec<Conversation>, String> {
    ...
    Ok(list.into_iter().map(|(id, title, created_at)| Conversation { id, title, created_at }).collect())
}
...
async fn delete_conversation(state: State<'_, AppState>, conversation_id: String) -> Result<(), String> {
    let lib = state.librarian.lock().await;
    lib.delete_conversation(&conversation_id).map_err(|e| e.to_string())
}

#[tauri::command]
async fn rename_conversation(state: State<'_, AppState>, conversation_id: String, new_title: String) -> Result<(), String> {
    let lib = state.librarian.lock().await;
    lib.rename_conversation(&conversation_id, &new_title).map_err(|e| e.to_string())
}
...
async fn generate_title(
    ...
    let history = lib_guard.get_complete_history(&conversation_id).map_err(|e| e.to_string())?;
    
    // Drop guard to allow async IO (request to LLM)
    drop(lib_guard);

    if history.is_empty() { return Ok("New Chat".to_string()); }

    let mut prompt = String::new();
    for (i, (role, content, _, _)) in history.iter().enumerate().take(6) {
    ...
    let tools = vec![]; 
    let messages = vec![Message { role: "user".to_string(), content: Some(prompt), tool_calls: None, tool_call_id: None }];
    ...
    let response = match provider_config.provider_type {
        ProviderType::OpenAI | ProviderType::OpenAICompatible => {
             ...
             provider.chat(messages, tools).await.map_err(|e| e.to_string())
        },
        ...
        ProviderType::Ollama => {
            let base_url = provider_config.base_url.clone().unwrap_or("http://localhost:11434".to_string());
            let provider = OllamaProvider::new(base_url, model.clone());
            provider.chat(messages, tools).await.map_err(|e| e.to_string())
        }
    }?;
    
    let new_title = response.content.unwrap_or("New Chat".to_string()).trim().trim_matches('"').to_string();
...
#[tauri::command]
async fn get_chat_history(state: State<'_, AppState>, conversation_id: String) -> Result<Vec<Message>, String> {
    let lib = state.librarian.lock().await;
    let raw = lib.get_complete_history(&conversation_id).map_err(|e| e.to_string())?;
...
async fn send_message(
    ...
    if !context_text.is_empty() {
        let context_msg = Message {
            ...
            content: Some(format!("You have access to the following long-term memories:\n{}", context_text)),
            tool_calls: None,
            tool_call_id: None,
        };
    ...
    // Prune context
    let pruned_messages = ContextManager::prune_messages(final_messages, 64000)
    ...
    let response = match provider_config.provider_type {
        ProviderType::OpenAI | ProviderType::OpenAICompatible => {
             ...
             let base_url = provider_config.base_url.clone();
             let provider = OpenAiProvider::new(api_key, base_url, model.clone());
             provider.chat(pruned_messages, tools).await.map_err(|e| e.to_string())
        },
        ProviderType::Anthropic => {
             let api_key = provider_config.api_key.clone().unwrap_or_default();
             let provider = AnthropicProvider::new(api_key, model.clone());
    ...
    match response {
        Ok(msg) => {
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(e) = attempt_pruning(app_handle, librarian_arc, provider_id_bg, model_bg, conv_id_bg).await {
...
async fn execute_tool(
    ...
    state.mcp_manager.call_tool(&name, args).await.map_err(|e| e.to_string())
}
...
#[tauri::command]
async fn fetch_models(provider_type: ProviderType, base_url: Option<String>, api_key: Option<String>) -> Result<Vec<ModelConfig>, String> {
    let client = reqwest::Client::new();
    let mut models = Vec::new();

    match provider_type {
        ProviderType::OpenAI | ProviderType::OpenAICompatible => {
            let url = format!("{}/v1/models", base_url.unwrap_or_else(|| "https://api.openai.com".to_string()).trim_end_matches('/'));
            ...
            if let Some(arr) = json["data"].as_array() {
                for m in arr {
                    if let Some(id) = m["id"].as_str() {
                        models.push(ModelConfig { id: id.to_string(), name: id.to_string(), visible: true });
                    }
                }
            }
        }
                ...
                if let Some(arr) = json["data"].as_array() {
                    for m in arr {
                        if let Some(id) = m["id"].as_str() {
                            ...
                            models.push(ModelConfig { id: id.to_string(), name, visible: true });
                        }
                    }
                }
            } else {
                 // Fallback if fetch fails (e.g. key invalid or network issue), but updated with latest models
                 models.push(ModelConfig { id: "claude-3-5-sonnet-20241022".to_string(), name: "Claude 3.5 Sonnet (New)".to_string(), visible: true });
                 models.push(ModelConfig { id: "claude-3-5-haiku-20241022".to_string(), name: "Claude 3.5 Haiku".to_string(), visible: true });
                 models.push(ModelConfig { id: "claude-3-5-sonnet-20240620".to_string(), name: "Claude 3.5 Sonnet (Old)".to_string(), visible: true });
                 models.push(ModelConfig { id: "claude-3-opus-20240229".to_string(), name: "Claude 3 Opus".to_string(), visible: true });
                 models.push(ModelConfig { id: "claude-3-sonnet-20240229".to_string(), name: "Claude 3 Sonnet".to_string(), visible: true });
                 models.push(ModelConfig { id: "claude-3-haiku-20240307".to_string(), name: "Claude 3 Haiku".to_string(), visible: true });
            }
        }
        ProviderType::Ollama => {
            ...
            if let Some(arr) = json["models"].as_array() {
                for m in arr {
                    if let Some(name) = m["name"].as_str() {
                         models.push(ModelConfig { id: name.to_string(), name: name.to_string(), visible: true });
                    }
                }
            }
        }
...
async fn add_mcp_server(
    ...
    state.mcp_manager.initialize(runtime_config).await.map_err(|e| e.to_string())
}
...
async fn attempt_pruning(
    ...
    let count = {
        ...
        lib.get_message_count(&conversation_id).map_err(|e| e.to_string())?
    };
    ...
    if count > limit {
        ...
        let oldest = {
            let lib = librarian.lock().await;
            lib.get_oldest_messages(&conversation_id, prune_count).map_err(|e| e.to_string())?
        };

        if oldest.is_empty() { return Ok(()); }
        ...
        if !provider_config.enabled { return Ok(()); } // Skip pruning if provider disabled
        ...
        match provider_response {
            Ok(msg) => {
                if let Some(summary) = msg.content {
                    ...
                    lib.save_summary(&conversation_id, &formatted_summary, &last_timestamp).map_err(|e| e.to_string())?;
                    lib.delete_messages(&ids_to_delete).map_err(|e| e.to_string())?;
